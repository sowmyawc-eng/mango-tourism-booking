...
import { Leaf, PartyPopper, Upload, X, Image } from 'lucide-react'
import toast from 'react-hot-toast'

// ── Fixed festival dates for 2026 ────────────────────────────────────────────
...
// ── Compress image to base64 (max 900px, JPEG 75%) ──────────────────────────
...
export default function PublicBookingForm() {
  ...
  // Load active POS locations for the dropdown
  useEffect(() => {
  ...
  async function handleReceiptChange(e) {
    ...
    if (file.size > 8 * 1024 * 1024) {
      toast.error('Image too large. Please use a file under 8 MB.')
      return
    }
    try {
      const b64 = await compressImage(file)
      // Guard: Firestore doc limit ~1 MB; base64 of 900px JPEG is ~150–350 KB
      if (b64.length > 900_000) {
        toast.error('Image still too large after compression. Try a smaller file.')
        return
      }
  ...
  async function onSubmit(data) {
    if (!receiptBase64) {
      toast.error('Please upload your UPI payment receipt screenshot')
      return
    }
    setSaving(true)
    try {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="card p-4 space-y-3">
            ...
            <p className="text-xs text-gray-500 -mt-1">
              Pay via UPI, then fill in the details below and upload your receipt.
            </p>
            ...
            <div>
              ...
              <input
                ...
                autoCapitalize="none"
                autoCorrect="off"
                {...register('upi_id', { required: 'Required' })}
              />
            ...
            <div>
              ...
              {!receiptBase64 ? (
                ...
                <div className="relative rounded-xl overflow-hidden border border-mango-200">
                  <img
                    src={receiptBase64}
                    alt="Receipt"
